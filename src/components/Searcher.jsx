            ...
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={ handleSearcher }>
                ...
                >
                    <div className="relative">
                        <Combobox.Input
                            className="h-16 w-full pl-4 pr-4 bg-gray-600 text-white placeholder-gray-400 text-lg"
                            placeholder="Buscar..."
                        ...
                        <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                            {
                                leakedProjects.map( project => (
                                    <Combobox.Option
                                        key={ project._id }
                                        value={ project }
                                        className={( {active} ) => classNames('cursor-default select-none px-4 py-2 h-12 text-lg', active && 'bg-sky-600 text-white') }
                                    >
                                        { project.name }
                                    </Combobox.Option>
                                ))
